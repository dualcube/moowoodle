...
class MooWoodle_Enrollment {
	...
	private function get_user_data( $moodle_user_id = 0 ) {
		...
		$moodle_pwd_meta = get_user_meta( $user_id, '_moodle_user_pwd', true );
		$pwd = '';
		if ( empty( $moodle_pwd_meta ) )  {
			$pwd = $this->password_generator();
			add_user_meta( $user_id, '_moodle_user_pwd', $pwd );
		} else {
			$pwd = $moodle_pwd_meta;
		}	
		$user_data = array();
	...
	private function enrol_moodle_user( $moodle_user_id, $suspend = 0 ) {
		...
		add_post_meta( $wc_order->id, 'enrolment_date', time() );
