...
class Enrollment {
	...
	private function get_moodle_user_id($create_moodle_user = false) {
		...
		if ($user_id) {
			$moodle_user_id_meta = MOOWOODLE_PRO_ADV || empty(get_user_meta($user_id, 'moowoodle_moodle_user_id')) ? get_user_meta($user_id, 'moowoodle_moodle_user_id') : wordpress_user_sync_to_moodle(get_userdata($user_id));
	...
	private function enrol_moodle_user($moowoodle_moodle_user_id, $suspend = 0) {
		...
		update_user_meta($wc_order->get_user_id(), 'moowoodle_moodle_user_id', $moowoodle_moodle_user_id);
	}
	/**
