...
class MooWoodle_PostType {
	...
	private function create_post_type_labels( $token, $singular, $plural, $menu ) {
		$labels = array(
			...
			'add_new'            => sprintf( _x( 'Add New %s', $token, MOOWOODLE_TEXT_DOMAIN ), $singular ),
			'add_new_item'       => sprintf( __( 'Add New %s', MOOWOODLE_TEXT_DOMAIN ), $singular ),
			'edit_item'          => sprintf( __( 'Edit %s', MOOWOODLE_TEXT_DOMAIN ), $singular ),
			'new_item'           => sprintf( __( 'New %s', MOOWOODLE_TEXT_DOMAIN ), $singular ),
			'all_items'          => sprintf( __( '%s', MOOWOODLE_TEXT_DOMAIN ), $plural ),
			'view_item'          => sprintf( __( 'View %s', MOOWOODLE_TEXT_DOMAIN ), $singular ),
			'search_items'       => sprintf( __( 'Search %s', MOOWOODLE_TEXT_DOMAIN ), $plural ),
			'not_found'          => sprintf( __( 'No %s found', MOOWOODLE_TEXT_DOMAIN ), strtolower( $plural ) ),
			'not_found_in_trash' => sprintf( __( 'No %s found in Trash', MOOWOODLE_TEXT_DOMAIN ), strtolower( $plural ) ),
			'parent_item_colon'  => ''
		);
		return $labels;
	}
	...
	private function create_post_type_messages( $post_type ) {
		...
		$messages = array(
			...
			2  => __( 'Custom field updated.', MOOWOODLE_TEXT_DOMAIN ),
			3  => __( 'Custom field deleted.', MOOWOODLE_TEXT_DOMAIN ),
			4  => sprintf( __( '%s updated.', MOOWOODLE_TEXT_DOMAIN ), esc_attr( $this->labels[ $post_type ]['singular' ] ) ),
			5  => isset( $_GET[ 'revision' ] ) ? sprintf( __( '%2$s restored to revision from %1$s', MOOWOODLE_TEXT_DOMAIN ), 
			...
			7  => sprintf( __( '%s saved.', MOOWOODLE_TEXT_DOMAIN ), esc_attr( $this->labels[ $post_type ][ 'singular' ] ) ),
			8  => sprintf( __( '%2$s submitted.', MOOWOODLE_TEXT_DOMAIN ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ), 
													esc_attr( $this->labels[ $post_type ][ 'singular' ] ) ),
			9  => sprintf( __( '%s scheduled for: <strong>%1$s</strong>.', MOOWOODLE_TEXT_DOMAIN ),
													date_i18n( __( ' M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ), 
													strtolower( esc_attr( $this->labels[ $post_type ][ 'singular' ] ) ) ),
			10 => sprintf( __( '%s draft updated.', MOOWOODLE_TEXT_DOMAIN ), esc_attr( $this->labels[ $post_type ][ 'singular' ] ) ),
		);
		return $messages;
	}
