...
class Course {
	 ...
	 */
	public function moowoodle_sync_courses_to_db( $courses ) {
		global $wpdb;
		...
		foreach ( $courses as $course ) {
			...
			// Increment sync count only for new inserts
			\MooWoodle\Util::increment_sync_count( 'course' );
			$updated_ids[] = $moodle_course_id;
		}
	...
	public static function migrate_courses() {
		...
		foreach ( $courses as $course ) {
			...
			// Optionally delete post and meta
