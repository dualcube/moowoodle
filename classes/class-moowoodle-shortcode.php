...
class MooWoodle_shortcode {
	...
	public function moowoodle_handler( $atts, $content = null ) {
		...
		if ( $content == null || ! is_user_logged_in() ) {
			...
			$url = '<a target="' . esc_attr( $target ) . '" class="' . esc_attr( $class ) . '" href="' . $MooWoodle->enrollment->moowoodle_generate_hyperlink( $course, $activity ) . '">' . do_shortcode( $content ) . '</a>'; // hyperlinked content
		}
		return $url;
	}
	...
	public function moowoodle_add_plugin( $plugin_array ) {
		global $MooWoodle;
	    $plugin_array[ 'moowoodle' ] = $MooWoodle->plugin_url . 'assets/admin/js/moowoodle.js';
	    return $plugin_array;
	}
}