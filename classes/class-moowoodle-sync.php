...
class MooWoodle_Sync {
	...
	*/
	private function sync_categories() {
		...
		$categories = moowoodle_moodle_core_function_callback( $MooWoodle->moodle_core_functions[ 'get_categories' ] );
	...
	private function update_categories( $categories, $taxonomy, $meta_key ) {
		...
		if ( ! empty( $categories ) ) {
			foreach ( $categories as $category ) {
				...
				if ( ! $term_id ) {
					...
					if ( ! is_wp_error( $term ) ) {
						apply_filters( "moowoodle_add_{$meta_key}_meta", $term[ 'term_id' ], '_category_id', $category[ 'id' ], false );
						apply_filters( "moowoodle_add_{$meta_key}_meta", $term[ 'term_id' ], '_parent', $category[ 'parent' ], false );
						apply_filters( "moowoodle_add_{$meta_key}_meta", $term[ 'term_id' ], '_category_path', $category[ 'path' ], false );
					}
				} else {
					...
					if ( ! is_wp_error( $term ) ) {
						apply_filters( "moowoodle_update_{$meta_key}_meta", $term[ 'term_id' ], '_parent', $category[ 'parent' ], '' );
						apply_filters( "moowoodle_update_{$meta_key}_meta", $term[ 'term_id' ], '_category_path', $category[ 'path' ], false );
					}
				}
		...
		if ( $terms ) {
			foreach ( $terms as $term ) {
				$category_id = apply_filters( "moowoodle_get_{$meta_key}_meta", $term->term_id, '_category_id', true );
				if ( in_array( $category_id, $category_ids ) ) {
					$parent = apply_filters( "moowoodle_get_{$meta_key}_meta", $term->term_id, '_parent', true );
	...
	*/
	private function sync_courses() {
		...
		$sync_settings = $MooWoodle->options_synchronize_settings;
		$courses = moowoodle_moodle_core_function_callback( $MooWoodle->moodle_core_functions[ 'get_courses' ] );
