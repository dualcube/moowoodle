  ...
  if ( count( $customer_orders ) > 0 ) {
      ...
      foreach ( $customer_orders as $customer_order ) {
        ...
        foreach ( $order->get_items() as $enrolment ) {
          ...
          $course_name = get_the_title( $enrolment->get_product_id() );          
          $linked_course_id = get_post_meta( $enrolment->get_product_id(), 'linked_course_id', true );
          
          $enrollment_list[] = get_moowoodle_course_url( $linked_course_id, $course_name );
          if ( $order->get_status() == 'completed' ) {
            ?> <p> <?php echo '<button type="button" class="button-tri">' . $enrollment_list[ $i ] . '</button> <br>'; ?> </p> <?php 
          } else {
            ?> <p> <?php echo '<div class="payment-tri">' . esc_html_e( "You can not access your course : ", 'moowoodle' ) . esc_html( $course_name ) . esc_html_e( "( Payment ", 'moowoodle' ) . " " . esc_html( $order->get_status() ) . ' )</div>'; ?> </p> <?php 
          }
          $i++;
