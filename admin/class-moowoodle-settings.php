	...
	*/
	public function add_settings_page() {
		...
		add_menu_page(
			...
			$MooWoodle->plugin_url . 'assets/images/dualcube.png',
      50
		);
            		...
            		<div id="post-body" class="metabox-holder columns-2">
              			...
              			<div id="postbox-container-1" class="postbox-container">
                			<div id="side-sortables" class="meta-box-sortables ui-sortable">
                				<?php if( apply_filters( 'moowoodle_free_active_side_adv', true ) ) { ?>
		                          ...
		                          target="_blank">
		                          						<?php _e('Got a Support Question', 'moowoodle') ?>
		                       						</a> 
  ...
  public function moowoodle_plugin_options_tabs() {
    ...
    if ($uses_tabs) {
      ...
      foreach ($MooWoodle->library->moowoodle_get_options() as $v) {
        ...
        if ($menu_slug == $page && $v[ 'type' ] == 'tab') {
          ...
          if ($v[ 'id' ] == 'moowoodle-from') {
            echo '<a id="'.$v[ 'id' ].'" class="nav-tab ' . $active . '" href="admin.php?moowoodle&tab=moowoodle-from">';
          } else {
      ...
      if( apply_filters( 'moowoodle_free_active', true ) ){
        echo '<a class="nav-tab moowoodle-upgrade" href="https://dualcube.com/shop/" target="_blank" rel="noopener noreferrer"><i class="dashicons dashicons-awards"></i> '.__('Upgrade to Pro for More Features', 'moowoodle').'</a>';
      }
  ...
  public function moowoodle_do_settings_sections($page, $show_submit) {
      ...
      foreach ((array) $wp_settings_sections[ $page ] as $section) {
        ...
        <p>
          ...
          ?>
              <input name="syncnow" type="submit" value="<?php _e('Sync Now', 'moowoodle'); ?>" class="button-primary" />
          <?php
            } elseif ($page == "moowoodle-link-course-table") {
              echo "Cannot find your course in this list?"
          ?>
              <a href="<?php echo get_site_url().'/wp-admin/admin.php?page=moowoodle-synchronization' ?>" ><?php _e('Synchronize Moodle Courses from here.', 'moowoodle'); ?></a>
                             
          <?php
            } else {
          ?>
              <input name="submit" type="submit" value="<?php _e('Save All Changes', 'moowoodle'); ?>" class="button-primary" />
          <?php
            }
          ?>
        </p>
  ...
  public function settings_page_init() { 
    ...
    foreach ($MooWoodle->library->moowoodle_get_options() as $k => $v) {
      switch ($v[ 'type' ]) {
        ...
        add_settings_field($v[ 'id' ], $v[ 'label' ], $v[ 'callback' ], $section_id, $section_id, apply_filters( 'moowoodle_add_settings_field', array(
          'id' => $v[ 'id' ],
