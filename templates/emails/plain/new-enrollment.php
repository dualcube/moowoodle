...
if ($count_course_order && $count_course_order < 2) {
	echo esc_html('Username : ', MOOWOODLE_TEXT_DOMAIN) . esc_html($user_details->data->user_login) . '\n\n';
	echo esc_html('Password : ', MOOWOODLE_TEXT_DOMAIN) . esc_html($pwd) . '\n\n';
}
echo esc_html('To enroll and access your course please click on the course link given below :', MOOWOODLE_TEXT_DOMAIN) . '\n\n';
...
if ($count_course_order && $count_course_order < 2) {
	echo esc_html('You need to change your password after first login.', MOOWOODLE_TEXT_DOMAIN) . '\n\n';
}